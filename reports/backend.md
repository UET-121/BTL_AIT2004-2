...
Vòng đời ứng dụng được quản lý tập trung thông qua hàm khởi tạo ngữ cảnh [lifespan](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/main.py#L21) trong [main.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/main.py):
- **Khi ứng dụng khởi động (Startup):**
  - Cấu hình logger hệ thống.
  - Thực hiện nạp trước (eager-load) các mô hình ML (YOLO và EasyOCR) vào bộ nhớ RAM/VRAM thông qua hàm `preload_ml_components` từ [factories.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/services/factories.py) nhằm loại bỏ độ trễ (latency spike) ở khung hình đầu tiên.
...
Mã nguồn định nghĩa tuyến đường (routing) nằm tại tệp [routes.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/api/routes.py).
...
Lớp [StreamManager](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/manager.py#L11) quản lý hai luồng chạy nền (Background Daemon Threads):
1. **LPR-CaptureThread ([capture_thread_fn](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/capture.py)):** Thực hiện đọc tuần tự các khung hình từ camera RTSP hoặc tệp video bằng OpenCV `cv2.VideoCapture` và ghi đè vào bộ đệm khung hình chung [shared_state](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/state.py).
2. **LPR-InferenceThread ([inference_loop_fn](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/inference.py#L113)):** Lấy khung hình mới nhất từ bộ đệm chung, thay đổi kích thước về độ phân giải chuẩn $1280x720$ để tiết kiệm năng lực CPU, chạy mô hình phát hiện biển số, theo vết, nhận diện OCR và gửi kết quả.
...
Vì các luồng chạy nền tính toán học máy là các tiến trình đồng bộ (Blocking Sync) nhằm tận dụng tối đa CPU, nhưng các cổng phát WebSocket và Database của FastAPI lại là bất đồng bộ (Async). Backend giải quyết bằng hàm helper [run_async](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/inference.py#L24):
...
Mã nguồn tích hợp lưu trữ tại [storage.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/services/storage.py). Lớp trừu tượng [StorageService](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/services/storage.py#L12) định nghĩa giao diện chung cho hai lớp triển khai:
...
Bộ động cơ cơ sở dữ liệu `create_async_engine` được cấu hình tối ưu hóa trong [database.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/shared/database.py#L12):
...
Hàm phụ thuộc [get_db](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/shared/database.py#L32) sử dụng cơ chế Generator bất đồng bộ (`async def` kết hợp `yield`):
...
* **Tệp khởi chạy chính:** [main.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/main.py) -> Quản lý lifespan ứng dụng.
* **Các tuyến dẫn HTTP API:** [routes.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/api/routes.py).
* **Quản lý đa luồng stream:** [manager.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/manager.py) -> Lớp [StreamManager](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/manager.py#L11).
* **Vòng lặp suy diễn thời gian thực:** [inference.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/inference.py) -> Hàm điều khiển [inference_loop_fn](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/inference.py#L113).
* **Quản lý kết nối WebSocket:** [broadcaster.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/broadcaster.py) -> Lớp [Broadcaster](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/realtime/broadcaster.py#L6).
* **Dịch vụ lưu trữ tệp tin:** [storage.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/services/storage.py) -> Lớp trừu tượng [StorageService](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/services/storage.py#L12).
* **Cơ chế nạp phiên cơ sở dữ liệu:** [database.py](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/shared/database.py) -> Bộ quản lý kết nối và hàm [get_db](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/app/shared/database.py#L32).
