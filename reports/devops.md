...
Hệ thống sử dụng **Docker Compose** làm công cụ chính để điều phối và quản lý toàn bộ các thành phần hạ tầng dưới dạng Monorepo. Toàn bộ cấu trúc orchestration được định nghĩa tại tệp [docker-compose.yml](file:///d:/MySC/Python/BTL_AIT2004-2/docker-compose.yml).
...
### 2.1. Container API Backend ([Dockerfile của API](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/Dockerfile))
...
### 2.2. Container Frontend ([Dockerfile của Frontend](file:///d:/MySC/Python/BTL_AIT2004-2/frontend/Dockerfile))
- **Base image:** Sử dụng `node:20-alpine` siêu nhẹ phục vụ môi trường chạy Node.js.
- **Quy trình cài đặt:** Sao chép các tệp cấu hình package, mã nguồn và tệp CSS chủ đề [default_shadcn_theme.css](file:///d:/MySC/Python/BTL_AIT2004-2/frontend/default_shadcn_theme.css), sau đó chạy `npm install` cấu hình cờ tự động thử lại 5 lần khi gặp lỗi mạng.
...
- **Giải pháp DevOps:** Cấu hình triệt tiêu overhead này bằng cách thiết lập các biến môi trường giới hạn luồng toán học về giá trị tĩnh `1` cho dịch vụ `api` trong file [docker-compose.yml](file:///d:/MySC/Python/BTL_AIT2004-2/docker-compose.yml#L47):
...
1. **PostgreSQL Healthcheck ([docker-compose.yml](file:///d:/MySC/Python/BTL_AIT2004-2/docker-compose.yml#L12)):** Dùng lệnh nội bộ `pg_isready` để kiểm tra cổng kết nối và database đã sẵn sàng nhận truy vấn chưa. Thăm dò mỗi 5 giây, thử lại tối đa 10 lần.
2. **MinIO Healthcheck ([docker-compose.yml](file:///d:/MySC/Python/BTL_AIT2004-2/docker-compose.yml#L63)):** Chạy lệnh `curl` gửi yêu cầu HTTP đến endpoint `/minio/health/live` để xác nhận MinIO đã khởi động hoàn toàn.
3. **API Dependency Control ([docker-compose.yml](file:///d:/MySC/Python/BTL_AIT2004-2/docker-compose.yml#L22)):** API chỉ được khởi động khi container cơ sở dữ liệu đạt trạng thái `service_healthy`. Điều này ngăn chặn lỗi sập kết nối cơ sở dữ liệu khi chạy các đoạn code tự động đồng bộ lược đồ DB lúc khởi chạy.
...
* **Tập tin orchestration chính:** [docker-compose.yml](file:///d:/MySC/Python/BTL_AIT2004-2/docker-compose.yml).
* **Dockerfile của backend API:** [Dockerfile (API)](file:///d:/MySC/Python/BTL_AIT2004-2/apps/api/Dockerfile).
* **Dockerfile của frontend web:** [Dockerfile (Frontend)](file:///d:/MySC/Python/BTL_AIT2004-2/frontend/Dockerfile).
* **Cấu hình biến môi trường mẫu:** [.env.example](file:///d:/MySC/Python/BTL_AIT2004-2/.env.example).
* **Tài liệu hướng dẫn biến môi trường:** [06-env-variables.md](file:///d:/MySC/Python/BTL_AIT2004-2/plan/_shared/06-env-variables.md).
* **Bảng theo dõi rủi ro hạ tầng:** [07-risk-register.md](file:///d:/MySC/Python/BTL_AIT2004-2/plan/_shared/07-risk-register.md).
